...
const tabelaINSS_2018 = [
  { ate: 1693.72, aliquota: 8 },
  { ate: 2822.9, aliquota: 9 },
  { ate: 5645.8, aliquota: 11 },
  { ate: Number.MAX_SAFE_INTEGER, aliquota: 11 }
];

const tabelaIRPF_2018 = [
  { ate: 1903.98, aliquota: 0, deducao: 0 },
  { ate: 2826.65, aliquota: 7.5, deducao: 142.8 },
  { ate: 3751.05, aliquota: 15.0, deducao: 354.8 },
  { ate: 4664.68, aliquota: 22.5, deducao: 636.13 },
  { ate: Number.MAX_SAFE_INTEGER, aliquota: 27.5, deducao: 869.36 }
];
...
export class Salario {
  ...
  _validarSalarioBruto(pSalarioBruto) {
    ...
    if (
      ...
      throw new Error(
        "O parâmetro do salário bruto é obrigatório e deve ser um valor do tipo number maior ou igual a 0!"
      );
    }
   ...
   */
  get salarioBruto() {
    return this._salarioBruto.toLocaleString("pt-br", formato);
  }

  get baseINSS() {
    return this._baseINSS.toLocaleString("pt-br", formato);
  }

  get descontoINSS() {
    return this._descontoINSS.toLocaleString("pt-br", formato);
  }

  get baseIRPF() {
    return this._baseIRPF.toLocaleString("pt-br", formato);
  }

  get descontoIRPF() {
    return this._descontoIRPF.toLocaleString("pt-br", formato);
  }

  get totalDescontos() {
    return (this._descontoINSS + this._descontoIRPF).toLocaleString(
      "pt-br",
      formato
    );
  }
  ...
  get salarioLiquido() {
    return this._salarioLiquido.toLocaleString("pt-br", formato);
  }
}
